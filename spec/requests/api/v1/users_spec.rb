...
RSpec.describe "Api::V1::Users", type: :request do
  ...
  let(:invalid_params) { { user: FactoryBot.attributes_for(:user, email: "") } }
  let(:new_attributes) { { user: FactoryBot.attributes_for(:user, first_name: "Jane") } }
  ...
  describe "PUT #update" do
    context "with valid params" do
      ...
      it "returns a success response" do
        put "/api/v1/users/#{user.id}", params: valid_params, headers: token
        expect(response).to have_http_status(:ok)
      end
    end
    ...
    context "with invalid params" do
      it "returns an unprocessable entity response" do
        put "/api/v1/users/#{user.id}", params: invalid_params, headers: token
        expect(response).to have_http_status(:unprocessable_entity)
      end
    end
  end
